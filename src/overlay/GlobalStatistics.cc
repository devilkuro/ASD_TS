...
namespace Fanjing {
GlobalStatistics* GlobalStatistics::ptr_singleton = NULL;
...
GlobalStatistics* GlobalStatistics::request() {
    if(ptr_singleton == NULL){
        ptr_singleton = new GlobalStatistics();
    }
    return ptr_singleton;
}

void GlobalStatistics::release() {
    if(ptr_singleton!=NULL){
