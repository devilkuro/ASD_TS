...
double ATSPeerInfo::getJoinScoreByDataSeq(unsigned int dataSeq) {
    if(freeResource > 0){
        ...
        //return 1/(1+0)*(lag+dataTimeStamp[dataSeq]);
        return result2score((1+0)*(lag+dataTimeStamp[dataSeq]));
        //加入时候无孩子节点（改）
    } else{
        EV<< "JoinScore:" <<0<<"\n";
        return 0;
...
double ATSPeerInfo::getInsertScoreByDataSeq(unsigned int dataSeq, double lagTF) {
    ...
    if(freeResource > 0){
        ...
        //return 1/((1+ChildNum_dataSeq[dataSeq])*(lagTF+lagT-lagF)+(dataTimeStamp[dataSeq]-lagF+lagTF));
        //因为自己的插入给原先孩子节点带来新的延时（改）
        return result2score((1+ChildNum_dataSeq[dataSeq])*(lagTF+lagT-lagF)+(dataTimeStamp[dataSeq]-lagF+lagTF));
    }
}

double ATSPeerInfo::getSwitchScoreByDataSeq(unsigned int dataSeq1, unsigned int dataSeq2, double lag1, double lag2,
        double lag3, double minlag1, double minlag2, double timeStampA, unsigned int childNo) {
    // dataSeq1 node D will deliver this data instead of B
    // dataSeq2 D get this data from B
    // lag1 lagBA
    // lag2 lagDA
    // lag3 DB
    // minlag1 lagDC
    // minlag2 lagBC
    // timeStampA A's time stamp of dataSeq1
    ...
    if (hasDataSeq1Child && hasDataSeq2Child){
        ...
        unsigned int parentChildNum1 = childlinklist[childNo]->childNum;
        unsigned int parentChildNum2 = ChildNum_dataSeq[dataSeq1];

        //double result1 = lag1 + minlag1 * parentChildNum1;
        //double result2 = lag2 + lag3 * 2 + minlag2 * parentChildNum1;
        double result1 = (lag1 + minlag2) *(parentChildNum1+1)+lag1*(1+parentChildNum2-(parentChildNum1+1));// B-C-tree + B-non-C-tree
        double result2 = (lag2 + minlag1) *(parentChildNum1+1)+(lag2 + lag3)*(1+parentChildNum2-(parentChildNum1+1));// D-C-tree + D-B-non-C-tree
        EV<<"switchdebugINFO:"
        <<"result1"<<","<<result1<<","
        <<"result2"<<","<<result2<<","
        <<"lag2"<<","<<lag2<<","
        ...
        return result2score((result2-result1)/2);
        //求倒数避免判定0,可能为负数
    } else{
        EV<< "SwitchScore:" <<0<<"\n";
        return 0;
