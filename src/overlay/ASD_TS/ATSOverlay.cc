...
{
    if (stage != MIN_STAGE_OVERLAY) return;  //第8阶段为overlay层
...
void ATSOverlay::handleATSQueryResponseMessage(ATSQueryResponseMessage *atsQueryResponseMsg)
{
    ...
    if(inputDegree<=serverFreeDegree){                                                       //服务器有多余出度，直接加入
        ...
        for(unsigned int i=0;i<atsQueryResponseMsg->getMemberAddressArraySize();i++){          //依次发送评估消息
    ...
    for(unsigned int i = 0;i<inputDegree;i++){//利用一个循环，每个出度都是分别添加，以下内容是每条支流单独加入的过程
        ...
        //若服务器没有多余节点则采取打分机制
            ...
            for(unsigned int j=0;j<PeerInfoList.size();j++){
                ...
                double insertScore = PeerInfoList[j]->getInsertScoreByDataSeq(i);//若采取Insert加入方式3，获得影响分数
                ...
                for(unsigned int k=i+1;k<inputDegree;k++){
                    if (parentLinkList[k]->getState() == ATS_LINK_INIT) {
                    	//得到父节点两条支流的孩子数量
                       unsigned int parentChildNum1=ChildNum_dataSeq[i];
                       unsigned int parentChildNum2=ChildNum_dataSeq[k];
                    	double tempSwitchScore =
                                PeerInfoList[j]->getSwitchScoreByDataSeq(i, k,parentChildNum1,parentChildNum2);  //若采取Switch加入方式2b，获得影响分数
...
void ATSOverlay::handleATSChildNumIncreaseMessage(ATSChildNumIncreaseMessage* atsChildNumIncreaseMsg)//增加对应支流的子节点数量
{
	...
	if(atsChildNumIncreaseMsg->getParentTargetAddress()==ServerAddress){
		...
		//这里从0开始
		ATSChildNumIncreaseMessage* atsChildNumIncreaseMsg_C = new ATSChildNumIncreaseMessage("atsChildNumIncreaseMsg_C");
		...
		atsChildNumIncreaseMsg_C->setParentTargetAddress(parentLinkList[atsChildNumIncreaseMsg->getDataSeq()]->getTargetAddress());//统一设置自身父节点
		sendATSMessageToUDP(atsChildNumIncreaseMsg_C,parentLinkList[atsChildNumIncreaseMsg->getDataSeq()]->getTargetAddress());
...
void ATSOverlay::updateVisualization(){
    ...
    for(unsigned int i = 0;i<childLinkList.size();i++){
        if(
        		childLinkList[i]->getDataSeq()==l
        		&&
        		childLinkList[i]->getState()==ATS_LINK_CONNECTED){
            showOverlayNeighborArrow(childLinkList[i]->getTargetAddress(),flag,newArrows[childLinkList[i]->getDataSeq()]);
            flag=false;
        }
    }
}
