...
void ATSOverlay::handleATSQueryResponseMessage(
		ATSQueryResponseMessage *atsQueryResponseMsg) {
	startTime = simTime() / SECOND - 2 * (simTime() / SECOND
			- atsQueryResponseMsg->getSendTime()); //获取开始时间
	...
	if (inputDegree <= serverFreeDegree) { //服务器有多余出度，直接加入
		...
		for (unsigned int i = 0; i
				< atsQueryResponseMsg->getMemberAddressArraySize(); i++) { //依次发送评估消息
...
void ATSOverlay::handleATSJoinEvalMessage(ATSJoinEvalMessage *atsJoinEvalMsg) {
	...
	ATSJoinEvalResponseMessage* atsJoinEvalResponseMsg =newATSJoinEvalResponseMessage("atsJoinEvalResponseMsg"); //新建评估反馈消息
	...
	for(unsigned int i = 0;i<inputDegree;i++) {//利用一个循环，每个出度都是分别添加，以下内容是每条支流单独加入的过程
		...
		//若服务器没有多余节点则采取打分机制
				...
				for(unsigned int k=i+1;k<inputDegree;k++) {
					if (parentLinkList[k]->getState() == ATS_LINK_INIT) {
						...
						if(PeerInfoList[j]->childlinklist.size()!=0) {//若B节点的孩子不为空
							...
							if(!PeerInfoList[j]->getChildAddressByDataSeq(k).isUnspecified()&& PeerInfoList[j]->getChildAddressByDataSeq(k)!=PeerInfoList[j]->getAddress()) {
								...
								for(unsigned int n=0;n<PeerInfoList[j]->childlinklist.size();n++) {
									...
									if(PeerInfoList[j]->childlinklist[n]->getDataSeq()==k) {
										...
										if(minLag1==0) {
											minLag1=getLagByAddress(PeerInfoList[j]->childlinklist[n]->getLinkOwnerAddress());
											tag=n;
											//在B的childlinklist里取C0到B的延时
											minLag2=PeerInfoList[j]->childlinklist[tag]->getLag();
										}
										//若B的孩子节点到D的延时更小（同时取到B的延时）
										if(minLag1>getLagByAddress(PeerInfoList[j]->childlinklist[n]->getLinkOwnerAddress())) {
						...
						PeerInfoList[j]->getSwitchScoreByDataSeq(i,k,lag1,lag2,lag3,minLag1,minLag2);//若采取Switch加入方式2b，获得影响分数
			...
			//SwitchSocre最高的话采取第2B种加入方案
				...
				TransportAddress pcAddress = PeerInfoList[maxSwitchNum]->childlinklist[tag]->getLinkOwnerAddress();//2014-12-17：replace fromPeerInfoList[maxSwitchNum]->getChildAddressByDataSeq(seq1);
...
void ATSOverlay::handleATSJoinMessage(ATSJoinMessage *atsJoinMsg) {
	if (freeDegree > 0) {
		...
		ATSJoinResponseMessage* atsJoinResponseMsg =
				new ATSJoinResponseMessage("atsJoinResponseMsg");
...
void ATSOverlay::checkNodeState() {
	...
	for(int i=0;i<parentLinkList.size();i++) {
		ATSChildNumIncreaseMessage* atsChildNumIncreaseMsg = new ATSChildNumIncreaseMessage("atsChildNumIncreaseMsg");
		atsChildNumIncreaseMsg->setDataSeq(i);
		atsChildNumIncreaseMsg->setParentTargetAddress(parentLinkList[i]->getTargetAddress());//设置目标父节点，主要用来控制到服务器节点停止向上反馈
		sendATSMessageToUDP(atsChildNumIncreaseMsg,parentLinkList[i]->getTargetAddress());//将消息发送给自己对应的父节点
...
void ATSOverlay::handleATSStatisticMessage(ATSStatisticMessage *atsStatisticMsg) {
	parentLinkList[atsStatisticMsg->getDataSeq()]->setLag(simTime() / SECOND
			- atsStatisticMsg->getSendTime());
...
void ATSOverlay::updateVisualization() {
	...
	unsigned int l = seq;
	for (unsigned int i = 0; i < childLinkList.size(); i++) {
		if (childLinkList[i]->getDataSeq() == l && childLinkList[i]->getState()
				== ATS_LINK_CONNECTED) {
			showOverlayNeighborArrow(childLinkList[i]->getTargetAddress(),
					flag, newArrows[0]
			//newArrows[i]
			);
			flag = false;
