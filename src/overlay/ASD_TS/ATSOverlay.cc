...
void ATSOverlay::finishOverlay() {
    ...
    if (nodeState == NodeState_Joined && nodeAddress != ServerAddress) {
        ...
        for (unsigned int i = 0; i < dataTimeStamp.size(); i++) {
            if (i == 0) {
                globalStatistics->recordOutVector("Fanjing:ATS:dataTimeStamp0",
                        dataTimeStamp[i]);
                globalStatistics->recordOutVector("Fanjing:ATS:dataHops0",
                        dataHops[i]);
                globalStatistics->recordOutVector("Fanjing:ATS:dataDataNum0",
                        getDataNumBySeq(i));
            } else if (i == 1) {
                globalStatistics->recordOutVector("Fanjing:ATS:dataTimeStamp1",
                        dataTimeStamp[i]);
                globalStatistics->recordOutVector("Fanjing:ATS:dataHops1",
                        dataHops[i]);
                globalStatistics->recordOutVector("Fanjing:ATS:dataDataNum1",
                        getDataNumBySeq(i));
            } else if (i == 2) {
                globalStatistics->recordOutVector("Fanjing:ATS:dataTimeStamp2",
                        dataTimeStamp[i]);
                globalStatistics->recordOutVector("Fanjing:ATS:dataHops2",
                        dataHops[i]);
                globalStatistics->recordOutVector("Fanjing:ATS:dataDataNum2",
                        getDataNumBySeq(i));
            }
            maxDataTimeStamp =
...
void ATSOverlay::handleATSQueryResponseMessage(
    ...
    startTime = simTime() / SECOND
            - 2 * (simTime() / SECOND - atsQueryResponseMsg->getSendTime());
...
void ATSOverlay::handleATSJoinEvalMessage(ATSJoinEvalMessage *atsJoinEvalMsg) {
    ...
    ATSJoinEvalResponseMessage* atsJoinEvalResponseMsg =
            new ATSJoinEvalResponseMessage("atsJoinEvalResponseMsg");
    ...
    for (unsigned int i = 0; i < parentLinkList.size(); i++) {
        atsJoinEvalResponseMsg->setParentAddress(i,
                parentLinkList[i]->getTargetAddress());
    }
    ...
    for (unsigned int i = 0; i < inputDegree; i++) {
        ...
        //Use peer nodes resource
            ...
            for (unsigned int j = 0; j < PeerInfoList.size(); j++) {
                ...
                double insertScore = PeerInfoList[j]->getInsertScoreByDataSeq(
                        i);
...
//                    maxSwitchSeq1 = maxSwitchSeq1 - maxSwitchSeq2;
                ...
                TransportAddress pcAddress =
                        PeerInfoList[maxSwitchNum]->getChildAddressByDataSeq(
                                seq1);
